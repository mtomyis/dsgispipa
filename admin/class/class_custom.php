...
class custom
{
	...
	function create_top_button($lvl,$name,$addr_root=''){
			global $addr_root;
            echo "<div class=\"btn-group\">";
            echo "<a class='btn btn-warning btn-sm' href='".$addr_root."home.php'><i class='icon-off'></i> TUTUP</a>";
            $level = $this->level_menu($lvl);   
            if((isset($level['C'])&&($level['C']=='1'))||($_SESSION['username']=='admin')){
                echo "<a class='btn btn-primary btn-sm' href='".$name."_add.php'><i class='icon-plus-sign'></i> TAMBAH</a>";
                    }
                    if((isset($level['R'])&&($level['R']=='1'))||($_SESSION['username']=='admin')){
                    echo "<a class='btn btn-success btn-sm' href='".$name.".php'><i class='icon-list-alt'></i> LIHAT</a>";
            }
            echo "</div>";
        ...
        function create_submit_button($attr=array('value'=>'PROSES')){
            echo "<input type='submit' name='submit_add' id='submit_add' value='".$attr['value']."' class='btn btn-success btn-sm'>";                   
        }
        ...
        function create_save_add_button($page_id,$attr=array('value'=>'SIMPAN')){
            ...
            if((isset($level['C'])&&($level['C']=='1'))||($_SESSION['username']=='admin')){
                    ...
                    echo "<button type='submit' name='submit_add' id='submit_add' class='btn btn-success btn-sm'><span class='glyphicon glyphicon-floppy-disk'> </span> ".$attr['value']."</button>";
            }
        }
        ...
        function create_save_button($lvl,$attr=array('value'=>'SIMPAN')){
            ...
            if((isset($level['U'])&&($level['U']=='1'))||($_SESSION['username']=='admin')){
                    ...
                    echo "<button type='submit' name='submit_edit' id='submit_edit' class='btn btn-success btn-sm'><span class='glyphicon glyphicon-floppy-saved'> </span> ".$attr['value']."</button>";
            }
        }
        ...
        function create_delete_button($lvl,$attr=array('value'=>'HAPUS')){
            ...
            if((isset($level['D'])&&($level['D']=='1'))||($_SESSION['username']=='admin')){
                    ...
                    echo "<button type='submit' name='submit_delete' id='submit_delete' class='btn btn-default btn-sm' onclick='return confirm(\"Yakin ingin menghapus data?\")'><span class='glyphicon glyphicon-trash'> </span> ".$attr['value']."</button>";
            }
        }
        
        function create_back_button($page_id=NULL,$attr=array('value'=>'KEMBALI')){
            echo "<button type='button' name='Back' id='Back' class='btn btn-default btn-sm' onclick='history.go(-1)'><span class='glyphicon glyphicon-arrow-left'> </span> ".$attr['value']."</button>";
        }
